...
- **Ad-platform CPM / CTR / CPA by period** — in Meta Ads Manager, Google Ads, or TikTok Ads Manager, open the campaign report and set the date range to *this period*, then duplicate it for the *prior equal period*; add CPM, CTR, CPA (or cost/result), conversions, and frequency as columns and export both windows side by side. Pull at the channel level first, then per campaign for the channels that moved.
- **Shopify / GA4 site CVR** — in Shopify, Analytics → Reports → "Online store conversion rate" (or the sessions-and-orders report) for both windows; in GA4, build an exploration with Sessions and Conversions (or use the e-commerce purchases report) and split add-to-cart → begin-checkout → purchase so a checkout-stage drop is visible. Match the date ranges exactly to the ad windows.
- **Tracking check** — reconcile platform-reported conversions against real orders: export purchases by source/UTM from Shopify (Orders, filtered by UTM or channel) and compare to each platform's reported conversions for the same window. A divergence that jumped past its normal baseline is the tracking-break signal; cross-check the pixel/CAPI/GA4/GTM change log and any checkout or consent-banner deploy.
- **Gotchas** — attribution windows and time zones differ across platforms (a 7-day-click Meta number won't equal a last-click GA4 number); platforms backfill conversions for days, so a fresh window understates conversions and overstates CPA; de-dupe pixel vs. CAPI before trusting platform counts; and keep both date ranges identical (same length, same weekday mix, no promo edge) or the comparison itself is the artifact.
